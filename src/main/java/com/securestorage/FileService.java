...
public class FileService {
    ...
    public void uploadFile(MultipartFile file, String ownerId) throws Exception {
        ...
        FileEntity entity = new FileEntity(fileId, ownerId, file.getOriginalFilename(), s3Key,
                encryptedAesKey, Base64.getEncoder().encodeToString(iv), file.getContentType());
    ...
    public List<FileEntity> listFiles() {
        return dynamoDb.table(tableName, TableSchema.fromBean(FileEntity.class)).scan().items().stream().collect(Collectors.toList());
    }
